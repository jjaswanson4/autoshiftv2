# trilio AutoShift Policy

## Overview
Installs the k8s-triliovault operator using AutoShift's ACM **PolicyGenerator** pattern. The
...
trilio/
  ...
  placement.yaml                # Placement predicate (autoshift.io/trilio) + tolerations
  manifests/
    operator-install/
...
hubClusterSets:
  hub:
    labels:
      trilio: 'true'
      trilio-subscription-name: 'k8s-triliovault'
      trilio-channel: 'lts-4.0'
      trilio-source: 'certified-operators'
      trilio-source-namespace: 'openshift-marketplace'
      # trilio-version: 'k8s-triliovault.v1.x.x'  # Optional: pin to a specific CSV
```
...
- **Pin a version** with the `trilio-version` label — a pinned version sets a manual
  install plan (`startingCSV` + `versions`), so upgrades become explicit.
```bash
# Find available CSV versions
oc get packagemanifests k8s-triliovault -o jsonpath='{.status.channels[*].currentCSV}'
```
     ...
     - complianceType: musthave
       objectDefinition:
         ...
         metadata:
           ...
           namespace: trilio-system
         spec:
           setting: '{{hub index .ManagedClusterLabels "autoshift.io/trilio-setting" | default "default-value" hub}}'
   ```
4. If the config must be its **own** Policy (separate remediation, a dependency, or a different
   ...
   `policy-trilio-operator-install`) and point it at the new manifest with its own
   `placement.placementPath`. `generate-policy.sh --dir policies/stable/trilio` scaffolds this.

Document any new `autoshift.io/trilio-*` label in `autoshift/values/clustersets/_example.yaml`
...
- **Policy not applied**: `oc get managedcluster <cluster> --show-labels`; check the Placement
  (`oc get placement -n policies-autoshift`) and `oc describe policy policy-trilio-operator-install`.
- **Operator install issues**: `oc get subscription,installplan -n trilio-system`;
...
## Resources
- [Operator Documentation](https://operatorhub.io/operator/k8s-triliovault)
